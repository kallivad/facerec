...
{
	double scalef = 1.0; //0.3;       //фактор сужения картинки для поиска лица мин. размера
	int score_thresh = 50;     // //100; // 50;  //ограничивающее число для локализации face box
	...
	try
	{
		...
		npd::npddetect npd;
	//	npd.load(argv[2]);
		npd.load("./fd_models/frontal_face_detector.bin");

	        // Load face detection and pose estimation models.
	        frontal_face_detector detector = get_frontal_face_detector();
	        shape_predictor pose_model;
	        dlib::deserialize("shape_predictor_68_face_landmarks.dat") >> pose_model;
	//	dlib::deserialize(argv[3]) >> pose_model;
	    ...
	    cv_image<bgr_pixel> fimg(frame);
			...
			for (int i = 0; i < n; i++)
			{
				...
				//рескалим найденный через npd fbox для того чтобы dlib мог расставить лэндмарки
			...
			if (faces.size() == 0)
			{
				cout << "Face detector(HOG): none face detected. Skipping frame!!! \n" << endl;
				win.clear_overlay();
				win.set_image(cimg);
				continue;
			}
			...
			for (int i = 0; i < in_descriptors.size(); i++)
			{
				for (int j = 0; j < face_descriptors.size(); j++)
				{
					...
					if (L < 0.6)
					{
						...
						id_person.push_back(detected_id);
						
						std::string notifier("Known face detected");
						cv::Point textPoint(10, 50);
						cv::putText(in, notifier, textPoint, cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 1.5, cv::Scalar::all(250), 2, 8);
						...
						cv::putText(in, user_id, textPoint, cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 1.5, cv::Scalar::all(250), 3, 8);
						cv::rectangle(in, cv::Rect(x,y,x1,y1), cv::Scalar(0, 0, 255), 2);
						
						
					} else 
					{ 
					  //подсветить неизвестных чувачковъ
					  cv::rectangle(in, cv::Rect(x,y,x1,y1), cv::Scalar(120,120,120), 1);
					}
				}
			}
