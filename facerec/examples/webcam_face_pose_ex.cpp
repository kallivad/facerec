...
void DB_load_descriptors(MYSQL *connection,std::vector<int>& Ids, std::vector<matrix<float, 0, 1>>& in_descriptors)
{
		...
		in_descriptors.erase(in_descriptors.begin(), in_descriptors.end());
		...
		{   
		  ...
		  {
		    std::cout  << "JSON Failed to parse" << reader.getFormattedErrorMessages();
		    //return 0;
		  }	
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		if(argc >= 4)
		  ...
		  cap = cv::VideoCapture(atoi(argv[1]));

		
		cout << "Video capture created " << endl;
		cap.cv::VideoCapture::set(cv::CAP_PROP_FRAME_WIDTH, 1280);
		cap.cv::VideoCapture::set(cv::CAP_PROP_FRAME_HEIGHT, 720);
	 ...
	 // Grab and process frames until the main window is closed by the user.
	    ...
	    //load descriptors from DB every N frames
	    if(frame_count%100 == 0)  
		DB_load_descriptors(connection, Ids, in_descriptors);
			...
			for (int i = 0; i < in_descriptors.size(); i++)
			{
				for (int j = 0; j < face_descriptors.size(); j++)
				{
					...
					if (L < 0.6)
					{
						...
						std::string user_id(std::to_string(Ids[detected_idx])); //i - is the id_user found						
